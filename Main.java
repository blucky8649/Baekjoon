...
public class Main {
	...
	public static Fish2[] fish; //물고기 정보 저장 
	...
	public static void main(String[] args) throws IOException {
		...
		for(int i = 0; i < 4; i++) {
			...
			for(int j = 0; j < 4; j++) {
				...
				fish[num] = new Fish2(num, i, j, dir, 1); 
	...
	public static void dfs(int sx, int sy, int sd, int eat) {
		...
		//fish 배열 복사 
		Fish2[] tempFish = new Fish2[fish.length];
		for(int i = 1; i <= 16; i++) 
			tempFish[i] = new Fish2(fish[i].num, fish[i].x, fish[i].y, fish[i].dir, fish[i].alive);
		...
		for(int i=1; i<=16; i++)
			fish[i] = new Fish2(tempFish[i].num, tempFish[i].x, tempFish[i].y, tempFish[i].dir, tempFish[i].alive);
	}
...
class Fish2 {
	...
	Fish2(int num, int x, int y, int dir, int alive) {
		this.num = num;
		this.x = x;
		this.y = y;
		this.dir = dir;
